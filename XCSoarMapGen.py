...
from List import WaypointList


# Waypoint_Strings = ["name,code,country,lat,lon,elev,style,rwdir,rwlen,freq,desc",
#              "Aarau Gare,Aarau Ga,,4723.500N,00803.117E,386.0m,1,,,,",
#              "Aarau Schachen C,Aarau Sc,,4723.276N,00802.057E,368.0m,1,,,,"]
with open('WaypointFile.cup') as f:
    Waypoint_Strings = f.readlines()

# for x in range(len(Waypoint_Strings)):
#     print(f'length = {len(Waypoint_Strings)}, entry = {Waypoint_Strings[x]}')
for wp_str in Waypoint_Strings:
    print(f'length = {len(wp_str)}, entry = {wp_str}')
...
my_waypoint_list = parse_seeyou_waypoints(Waypoint_Strings)
for wp in my_waypoint_list:
    print(wp)

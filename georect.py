...
class GeoRect:
    ...
    def intersects(self, other):
        return (
            self.inside(other.top, other.left)
            or self.inside(other.top, other.right)
            or self.inside(other.bottom, other.left)
            or self.inside(other.bottom, other.right)
            or other.inside(self.top, self.left)
            or other.inside(self.top, self.right)
            or other.inside(self.bottom, self.left)
            or other.inside(self.bottom, self.right)
        )

    def __get_projected_point(self, point, distance, direction):
        lat1 = point[0] * (math.pi / 180.0)
        lon1 = point[1] * (math.pi / 180.0)
        distance_rad = (distance / 1.852) * math.pi / (180.0 * 60.0)
    ...
    def expand(self, distance):
        midlat = (self.top + self.bottom) / 2.0
        midlon = (self.top + self.bottom) / 2.0
        ...
        bottom = self.__get_projected_point([self.bottom, midlon], distance, 180)
    ...
    def get_center(self):
        return GeoPoint((self.left + self.right) / 2.0, (self.top + self.bottom) / 2.0)